...
& $taskDotnet build src\airyPDF\airyPDF.csproj -c Release -p:RestoreLockedMode=true
if ($LASTEXITCODE -ne 0) { throw 'Build failed' }
if ($Test) {
    & $taskDotnet src\airyPDF\bin\Release\net10.0-windows\win-x64\airyPDF.dll --self-test
    if ($LASTEXITCODE -ne 0) { throw 'PDF and printing tests failed; see artifacts/test-failure.txt' }
    & $taskDotnet src\airyPDF\bin\Release\net10.0-windows\win-x64\airyPDF.dll --ui-test
    if ($LASTEXITCODE -ne 0) { throw 'UI tests failed; see artifacts/test-failure.txt' }
}
if ($Publish) {
    & $taskDotnet publish src\airyPDF\airyPDF.csproj -c Release -r win-x64 --self-contained true -o artifacts\app -p:PublishReadyToRun=true
