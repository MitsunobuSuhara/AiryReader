...
p=doc.add_paragraph(); p.alignment=WD_ALIGN_PARAGRAPH.CENTER
r=p.add_run("私はプログラムコードを一度も書いたことがありません。\nそれでもAIとの会話を通じて、仕事用のアプリやプラグインをいくつか作ってきました。\nこれまではClaudeを使い、最近はChatGPTのCodexを中心に開発しています。AiryReaderは、昨日か一昨日の朝にPDFリーダーの枠組みができ、昨日は細かな改善やメモ機能などを追加しました。私が実際に会話していた時間は、肌感覚では合計2〜3時間ほどです。")
r.font.size=Pt(13)
p=doc.add_paragraph(); p.alignment=WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph("昨日か一昨日の朝には、PDFを開いて原寸を意識して印刷できる、PDFリーダーの基本的な枠組みができていました。昨日は実際に使いながら細かな調整を重ね、メモ機能なども追加しました。私が実際にAIと会話していた時間は、肌感覚では合計2〜3時間ほどです。私が一日中パソコンに向かってプログラムを書いたわけではありません。仕事や用事の合間に、気づいたことをチャットで少し話しました。その間にAIが作成や修正、確認を進め、できたものを私が触って、また感想を返すという流れでした。私の感覚では、片手間に何度か話しかけていたら、使えるアプリが少しずつ出来上がってきた、というのが近いです。")
